...
- **kiro-cli** 已安装并完成认证（`kiro-cli auth`）
...
构建产物 `dist/acp-bridge`（~15MB），无需 Python 环境即可运行。

> **注意**：PyInstaller 不支持交叉编译，打包产物只能在构建时的操作系统上运行。如需在不同平台使用，需在对应平台（Windows WSL / macOS / Linux）上分别执行 `./build.sh` 构建。
